...
`stats::density`. Its new functionality is:
  ...
  - `kdensity` has built-in support for many parametric starts, such as
    `normal` and `gamma`, but you can also supply your own.
  - In addition to support for asymmetric kernels ones such as `gcopula`
    and `gamma` kernels, the common symmetric ones are also supported.
    You can also supply your own kernels.
  - A larger selection of choices for the bandwidth `bw`, again
